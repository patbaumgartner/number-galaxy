...
export function SpeedPhase({ planetId }: { planetId: PlanetId }) {
    ...
    if (finished) return <div className="page trainer-page"><Navigation /><main className="shell"><header className="trainer-header"><h2>{planet.emoji} {planet.label} - {t.tt.phaseSpeed}</h2><button className="btn btn--ghost btn--sm" onClick={() => navigate('/times-tables')}>{t.tt.trainExit}</button></header><div className="trainer-body"><SessionSummary phase="speed" planetId={planetId} accuracy={accuracy} streak={0} leveledUpCount={0} earnedStars={earnedStars} starsChanged={starsChanged} timeMs={totalMs} isNewBest={isNewBest} /></div></main></div>
    if (!started) return <div className="page trainer-page"><Navigation /><main className="shell"><header className="trainer-header"><h2>{planet.emoji} {planet.label} - {t.tt.phaseSpeed}</h2><button className="btn btn--ghost btn--sm" onClick={() => navigate('/times-tables')}>{t.tt.trainExit}</button></header><div className="trainer-body"><div className="panel speed-ready-card"><h2>{countdown > 0 ? countdown : t.tt.speedGo}</h2></div></div></main></div>
