...
export function PracticePhase({ planetId }: { planetId: PlanetId }) {
    ...
    if (finished) {
        return <div className="page trainer-page"><Navigation /><main className="shell"><header className="trainer-header"><h2>{planet.emoji} {planet.label} - {t.tt.phasePractice}</h2><button className="btn btn--sm" onClick={() => navigate('/times-tables')}>{t.tt.trainExit}</button></header><div className="trainer-body"><SessionSummary phase="practice" planetId={planetId} accuracy={accuracy} streak={bestStreak} leveledUpCount={leveledUpCount} earnedStars={earnedStars} starsChanged={starsChanged} /></div></main></div>
    }
